...
protected:
	...
	bool _onkeydown_firefox(int key, long data);
	...
	// NS XPCOM 
	nsCOMPtr<nsISearchEngine> NS_getSearchEngineByNum(unsigned int num);
	nsCOMPtr<nsISearchEngine> NS_getCurrentSearchEngine();

	bool NS_isInitialized() { return m_nsInitialized; };
	nsCOMPtr<nsIServiceManager> NS_getIServiceManager() { if(NS_isInitialized())return m_nsServMan; }
	nsCOMPtr<nsIBrowserSearchService> NS_getIBrowserSearchService() { if(NS_isInitialized())return m_nsSearchService; }
...
private:
	...
	nsCOMPtr<nsIServiceManager> m_nsServMan;
	nsCOMPtr<nsIBrowserSearchService> m_nsSearchService;
	nsCOMPtr<nsISupportsCString> m_nsSupportsCString_1; 
	nsCOMPtr<nsIObserverService> m_nsObserverService; 
	bool m_nsInitialized;
};
