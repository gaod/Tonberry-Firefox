...
// FIXME: This doesn't work under Firefox 1.5

#define	CTRL(x)		(x-'@')
static char important_mozilla_hot_keys[] = {
	CTRL('T'), 
	CTRL('1'),
	CTRL('2'),
	CTRL('3'),
	...
	case WM_IME_CHAR:
		{
			OnImeChar(wp, lp);
			/*
			if( strchr(important_mozilla_hot_keys, db[0]) )
				SendMessage( GetParent(GetParent(m_hWnd)), msg, wp, lp);
			*/
			return 0;
		}
	case WM_CHAR:
		...
		/*
		if( !strchr(important_mozilla_hot_keys, wp) )
			SendMessage( GetParent(GetParent(m_hWnd)), msg, wp, lp);
		*/
		return 0;
	case WM_SOCKET:
...
static void OpenWithMozilla( string url ) {
	...
	ShellExecute( NULL, "open", exe_path, url.c_str(), NULL, SW_SHOW );
}
...
{
	if ( _onkeydown_firefox(key, data) )
		return true;
...
bool CTelnetView::_onkeydown_firefox(int key, long data)
{
...
void CTelnetView::OpenShorterUrl(string url)
{
	...
	str_replace(terms, " ", "+");
	string post = "-new-tab " + url + "/" + terms;

	TCHAR exe_path[MAX_PATH];
	GetModuleFileName(NULL, exe_path, MAX_PATH );
	ShellExecute( NULL, "open", exe_path, post.c_str(), NULL, SW_SHOW );
}
...
{
	static bool _duplicate = false; //Workaround
	...
	if ( GET_KEYSTATE_WPARAM(wp) & MK_RBUTTON )
	{
		if ( m_keyFuncSwitch )
		{
			...
			if ( GET_WHEEL_DELTA_WPARAM(wp) / WHEEL_DELTA > 0  && !_duplicate ){
				SetFocus( GetParent(GetParent(m_hWnd) ));
				keybd_event(VK_CONTROL,0,0,0);
				keybd_event(VK_PRIOR,0,0,0);
				keybd_event(VK_CONTROL,0,KEYEVENTF_KEYUP,0);
				keybd_event(VK_PRIOR,0,KEYEVENTF_KEYUP,0);
			}else if(GET_WHEEL_DELTA_WPARAM(wp) / WHEEL_DELTA < 0  && !_duplicate){
				SetFocus( GetParent(GetParent(m_hWnd) ));
				keybd_event(VK_CONTROL,0,0,0);
	...
	if ( GetCon()->m_Site.m_UseMouseWheelHack )
...
// 2008.07.06 Added by neopro@ptt.cc
bool CTelnetView::NS_init()
{
		m_nsInitialized = false;
		nsresult rv;

        //nsCOMPtr<nsIServiceManager> m_servMan;
        rv = NS_GetServiceManager(getter_AddRefs(m_nsServMan));
        if (NS_FAILED(rv)) {
                return false;
        }

        //nsCOMPtr<nsIBrowserSearchService> m_searchService;
        rv = m_nsServMan->GetServiceByContractID( "@mozilla.org/browser/search-service;1" , NS_GET_IID(nsIBrowserSearchService),
				getter_AddRefs(m_nsSearchService));
        if (NS_FAILED(rv)) {
                return false;
        }
		...
		/*
		nsCOMPtr<nsIPrefService> nsPrefService;
        rv = m_nsServMan->GetServiceByContractID( "@mozilla.org/preferences-service;1" , NS_GET_IID(nsIPrefService),
				getter_AddRefs( nsPrefService) );
        if (NS_FAILED(rv)) {
                return false;
        }*/
		...
		m_nsServMan->GetServiceByContractID("@mozilla.org/observer-service;1", NS_GET_IID(nsIObserverService),
			getter_AddRefs(m_nsObserverService)); 


		//nsCOMPtr<nsISupportsCString> m_nsSupportsCString; 
		m_nsServMan->GetServiceByContractID("@mozilla.org/supports-cstring;1", NS_GET_IID(nsISupportsCString),
			getter_AddRefs(m_nsSupportsCString_1)); 


		m_nsInitialized = true;
		return true;
}

// 2008.07.06 Added by neopro@ptt.cc
nsCOMPtr<nsISearchEngine> CTelnetView::NS_getSearchEngineByNum(unsigned int num )
{
		PRUint32 count=0;
		nsCOMPtr<nsISearchEngine>* a = nsnull;

		nsresult rv = NS_getIBrowserSearchService()->GetEngines( &count, (nsISearchEngine***)&a );

		if (NS_SUCCEEDED(rv) && num<=count && num>=0)
		{	
			for ( unsigned int i=0; i<count; i++)
				getter_AddRefs(a[num]);
			return a[num];			
		}
		return nsnull;
}

nsCOMPtr<nsISearchEngine> CTelnetView::NS_getCurrentSearchEngine()
{
		nsCOMPtr<nsISearchEngine> engine;
		nsresult rv = (NS_getIBrowserSearchService())->GetCurrentEngine( getter_AddRefs(engine) );
        if (NS_SUCCEEDED(rv) || engine != nsnull){
			return engine;
		}
		return nsnull;
}

// 2008.07.06 Added by neopro@ptt.cc   .This is for TEST only.
void CTelnetView::NS_openWithSearchEngine(int action)
{
        nsresult rv;
		...
		/*
		// get the currently active search engine from Mozilla/Firefox.
		nsCOMPtr<nsISearchEngine> engine;
		rv = (NS_GetIBrowserSearchService())->GetCurrentEngine( getter_AddRefs(engine) );
        if (NS_FAILED(rv) || engine == nsnull){
			MessageBox( m_hWnd, "faild 3.", "Error",  MB_ICONSTOP|MB_OK );
			return;
		}*/
		nsCOMPtr<nsISearchEngine> engine;
		...
		if ( action == -1)
		{
			engine = NS_getCurrentSearchEngine();
			if(!engine)return;
		}
		else {
			engine= NS_getSearchEngineByNum(action);
			if(!engine)return;
		}

		// Encoding Convertion.
		string terms = this->m_pTermData->GetSelectedText().c_str();

		wchar_t* pwbuf = AnsiToUTF16( terms, this->GetCodePage() );

		nsEmbedString aTerms;
		NS_StringSetData( aTerms, pwbuf);
		delete [] pwbuf;

		// get the SearchSubmission object
		nsCOMPtr<nsISearchSubmission> sb;
		engine->GetSubmission( aTerms ,nsEmbedString(nsnull), getter_AddRefs(sb));

		// get URI form currently active search engine
		nsCOMPtr<nsIURI> nsuri;
		sb->GetUri( getter_AddRefs(nsuri) );		
		nsCString acUri;
		// *Encoded with ASCII
		// nsuri->GetAsciiSpec( acUri );
		// *Encoded with UTF8
		nsuri->GetSpec( acUri );

		char* pbuf = new char [acUri.Length()+1];
		memset(pbuf, 0, (acUri.Length()+1)*sizeof(char));
		NS_CStringGetData( acUri, (const char**)&pbuf);
		string sUri = pbuf;
		delete []pbuf;

		string post = "-new-tab \"" + sUri;

		// FIXME: The Http POST method is not implemented yet.
		//-------------------------------------------
		nsCOMPtr<nsIInputStream> is;
		rv = sb->GetPostData( getter_AddRefs(is) );
        if (NS_FAILED(rv)) {
			MessageBox( m_hWnd, "failed 3.", "Error",  MB_ICONSTOP|MB_OK );
                return;
        } 
		...
		if ( is != nsnull )
		{
			...
			char* postdata = new char [ (ibytes/sizeof(char))+1 ];
			memset( postdata, 0, ibytes+1*sizeof(char) );
			PRUint32 iend;
			is->Read( postdata, (ibytes)+1*sizeof(char), &iend);
			...
			if (NS_FAILED(rv))
				MessageBox( m_hWnd, "Create ioService failed.", "Error",  MB_ICONSTOP|MB_OK );

			nsCOMPtr<nsIURI> ext_uri;
			ioService->NewURI(acUri, nsnull,  nsnull, getter_AddRefs(ext_uri));


			nsCOMPtr<nsIChannel> channel;
			rv = ioService->NewChannelFromURI(ext_uri, getter_AddRefs(channel));
			if (NS_FAILED(rv))
				MessageBox( m_hWnd, "NewChannelFromURI failed.", "Error",  MB_ICONSTOP|MB_OK );


			nsCOMPtr<nsIUploadChannel> up_channel;
			up_channel = do_QueryInterface(channel);
			rv = up_channel->SetUploadStream( is, nsEmbedCString(nsnull), -1);
			if (NS_FAILED(rv))
				MessageBox( m_hWnd, "nsIUploadChannel failed.", "Error",  MB_ICONSTOP|MB_OK );
			MessageBox( m_hWnd, postdata, "Error",  MB_ICONSTOP|MB_OK );
			MessageBox( m_hWnd, sUri.c_str(), "Error",  MB_ICONSTOP|MB_OK );

			nsCOMPtr<nsIHttpChannel> http_channel;
			nsCOMPtr<nsIInputStream> http_is;
			http_channel = do_QueryInterface(channel);
			//http_channel->SetRedirectionLimit(0);
			//http_channel->SetAllowPipelining( true );
			http_channel->SetRequestMethod( nsEmbedCString("POST") );

			rv = channel->Open( getter_AddRefs(http_is) );
			if (NS_FAILED(rv))
				MessageBox( m_hWnd, "channel.Open failed.", "Error",  MB_ICONSTOP|MB_OK );

			// Get the Response Status Text
			nsEmbedCString retText;
			http_channel->GetResponseStatusText( retText );
			MessageBox( m_hWnd, retText.BeginReading(), "Error",  MB_ICONSTOP|MB_OK );

			http_is->Available( &ibytes );
			char* retdata = new char [ (ibytes/sizeof(char))+1 ];
			memset( retdata, 0, ibytes+1*sizeof(char) );
			http_is->Read( retdata, (ibytes)+1*sizeof(char), &iend);
			MessageBox( m_hWnd, retdata, "Error",  MB_ICONSTOP|MB_OK );
			delete [] retdata;*/

			// FIXME: The Http POST method is not implemented yet.
			char* p = postdata;
			p += (ibytes/sizeof(char))+1;
			char* buf = new char[(ibytes/sizeof(char))+1];
			memset( buf, 0, ibytes);
...
void CTelnetView::OpenWithSearchEngine( _SearchInfo* info ){
	...
	if ( !info->param_name.empty() )
		post = "-new-tab " + info->url + "?" + info->param_name + "=" + terms;
	else{
		post = "-new-tab " + info->url + "/" + terms;
	}
