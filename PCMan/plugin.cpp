...
/*
  // Security check: Only URLs started with telnet:// are allowed to use this plug-in
  securityCheckOK = false;

  nsIDOMWindow* domWindow;
  NPN_GetValue( mInstance, NPNVDOMWindow, 
  ...
  if (domWindow)  {
    ...
    if( NS_SUCCEEDED( domWindow->GetDocument( &doc ) ) )
    {
      ...
      if( NS_SUCCEEDED( doc->QueryInterface( NS_GET_IID(nsIDOMHTMLDocument), (void**)&htmlDoc ) ) )
      {
        nsEmbedString url;
        htmlDoc->GetURL( url );
        if( 0 == wcsncmp( L"telnet:", (const wchar_t*)url.get(), 7) )
        {
          securityCheckOK = true;  // This plug-in is loaded from telnet://
        }
...
static LRESULT CALLBACK PluginWinProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SIZE:
    ...
    break;
    default:
    ret = CallWindowProc( pi->m_OldWndProc, hWnd, msg, wParam, lParam );
      break;
  }
  return ret;
}
