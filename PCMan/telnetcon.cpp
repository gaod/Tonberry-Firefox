...
void CTelnetCon::OnClose(int code)
{
	...
	{
		FireEvent( nsnull, "pm_connection_close" );
	}
}
...
{
	FireEvent( nsnull, "pm_telnet_receive_BEL" );
}
...
//
void CTelnetCon::FireEvent(nsCOMPtr<nsISupports> subject, string topic)
{
	nsresult rv;
	...
	servMan->GetServiceByContractID("@mozilla.org/observer-service;1", NS_GET_IID(nsIObserverService),
		getter_AddRefs(observerService)); 
	if (NS_FAILED(rv))
		return;


	long lh = long(m_pView->m_hWnd);
	char buf[16];
	ltoa(lh, buf, 10);
	wchar_t wh[16];
	...
	observerService->NotifyObservers( subject, topic.c_str(), wh );
}